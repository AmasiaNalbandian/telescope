...
const SearchResults = () => {
  ...
  const { author, post, endDate, startDate, toggleHelp } = useSearchValue();
  const [totalPosts, setTotalPosts] = useState(0);

  const prepareUrl = (index: number) =>
    `http://localhost:4445/?postText=${encodeURIComponent(post)}&authorText=${encodeURIComponent(
      author
    )}&startDate=${startDate}&endDate=${endDate}&page=${index}`;

  // We only bother doing the request if we have something to search for.
  const shouldFetch = () => post.length > 0 || author.length > 0;
